...
sorted_list = sorted(staff_broken, key=lambda x: (x[0], x[1]))
result = defaultdict(set)

for department, name in sorted_list:
    result[department].add(name)

for department, names in sorted(result.items()):
    sorted_names = sorted(names)
    print(f'{department}: {", ".join(sorted_names)}')
