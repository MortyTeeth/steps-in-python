...
def two_digit_numbers(x):
    return 9 < abs(x) < 100


def divisible_by_7_without_remainder(x):
    return x % 7 == 0
...
print(sum(map(square, (filter(divisible_by_7_without_remainder, filter(two_digit_numbers, numbers))))), sep='\n')
